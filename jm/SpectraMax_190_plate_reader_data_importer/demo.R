...
# library(plyr)
# library(stringr)
# library(scales)  # 141221 for ggplot2's scale_y_continuous(labels = percent_format())
# #library(plot3D)
# # library(relaimpo)
# # for things like str_split_fixed(df$col, "_split_", 2)

##### PREPARE RAW absorbance & absorbance spectra data DATA #######
rm(fun_import_raw_spectra)
fun_import_raw_spectra <- function(filename, path) {
        ...
        # convert the time strings to elapsed times
        # there are some NA values: is.na(tail(d$time))
        # 150506: this ONLY works for time formats like "30:10" for 30 min 10 sec.  Can't do 1:04:22 for 1 hr, 4 min, 22 sec yet!
        ## Don't apply this function here; doesn't work for times > 1hr
        #         d$minutes <- sapply(strsplit(as.character(d$time), ":"),  function(x) { x <- as.numeric(x)
#                                                                                   x[1]+x[2]/60   })
        return(d)
}
...
filename_CSV
# d <- fun_import_raw_spectra(list.files("./raw_data"), "./raw_data")
rm(d)
d <- fun_import_raw_spectra(filename_CSV, "./raw_data")

tmp <- subset(d, scan.well == "A1")
tmp

time.col <- read.csv("./raw_data/2015_05_15 toy plate reader data to make importing function - time col only.csv",
                     stringsAsFactors = F)
head(time.col)
time.col
time.col[2,]
strsplit(as.character(time.col[2,]), ":")
time.col
class(time.col)
typeof(time.col)
time.col[3,]
as.character(time.col) # ?? What's up with this? "c(\"0:00\", \"12:00\", \"24:00:00\", \"36:00:00\", \"48:00:00\", \"1:00:00\")"
strsplit(time.col[2,], ":")
sapply(strsplit(time.col, ":"),  function(x) { x <- as.numeric(x)
                                                            x[1]+x[2]/60   })
#############################
##########  OLD  ############
#############################
