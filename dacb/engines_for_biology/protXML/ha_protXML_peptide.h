...
#define PROTXML_PEPTIDE_DEFAULT_ROOT_ALLOC 8192
...
typedef struct st_protXML_peptide_share {
	char *table_name;
	...
	struct st_protXML_peptide_share_position {
		int end;
	} filepos;
	...
	THR_LOCK lock;
} PROTXML_PEPTIDE_SHARE;

class ha_protXML_peptide: public handler {
	THR_LOCK_DATA lock;	/* MySQL based lock for handler */
	PROTXML_PEPTIDE_SHARE *share;	/* shared lock */

	int init_data_file();
	File protXML_peptide_file;
...
public:
	ha_protXML_peptide(handlerton *hton, TABLE_SHARE *table_arg);

	~ha_protXML_peptide();

	const char *table_type() const { return "PROTXML_PEPTIDE"; }
...
enum protXML_peptide_field { GROUP_NUMBER = 0, PSEUDO_NAME = 1, PROBABILITY = 2, DONE = 3 };

struct protXML_peptide_field_info {
	const char *fieldname;
	enum protXML_peptide_field state;
	enum protXML_peptide_field next_state;
	uint type;
	const char *type_text;
} protXML_peptide_fields[] = { \
