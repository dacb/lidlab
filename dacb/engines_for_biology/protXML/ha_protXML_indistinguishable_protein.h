...
#define PROTXML_INDISTINGUISHABLE_PROTEIN_DEFAULT_ROOT_ALLOC 8192
...
typedef struct st_protXML_indistinguishable_protein_share {
	char *table_name;
	...
	struct st_protXML_indistinguishable_protein_share_position {
		int end;
	} filepos;
	...
	THR_LOCK lock;
} PROTXML_INDISTINGUISHABLE_PROTEIN_SHARE;

class ha_protXML_indistinguishable_protein: public handler {
	THR_LOCK_DATA lock;	/* MySQL based lock for handler */
	PROTXML_INDISTINGUISHABLE_PROTEIN_SHARE *share;	/* shared lock */

	int init_data_file();
	File protXML_indistinguishable_protein_file;
...
public:
	ha_protXML_indistinguishable_protein(handlerton *hton, TABLE_SHARE *table_arg);

	~ha_protXML_indistinguishable_protein();

	const char *table_type() const { return "PROTXML_INDISTINGUISHABLE_PROTEIN"; }
...
enum protXML_indistinguishable_protein_field { GROUP_NUMBER = 0, PSEUDO_NAME = 1, PROBABILITY = 2, DONE = 3 };

struct protXML_indistinguishable_protein_field_info {
	const char *fieldname;
	enum protXML_indistinguishable_protein_field state;
	enum protXML_indistinguishable_protein_field next_state;
	uint type;
	const char *type_text;
} protXML_indistinguishable_protein_fields[] = { \
