...
#define PROTXML_MOD_AMINOACID_MASS_DEFAULT_ROOT_ALLOC 8192
...
typedef struct st_protXML_mod_aminoacid_mass_share {
	char *table_name;
	...
	struct st_protXML_mod_aminoacid_mass_share_position {
		int end;
	} filepos;
	...
	THR_LOCK lock;
} PROTXML_MOD_AMINOACID_MASS_SHARE;

class ha_protXML_mod_aminoacid_mass: public handler {
	THR_LOCK_DATA lock;	/* MySQL based lock for handler */
	PROTXML_MOD_AMINOACID_MASS_SHARE *share;	/* shared lock */

	int init_data_file();
	File protXML_mod_aminoacid_mass_file;
...
public:
	ha_protXML_mod_aminoacid_mass(handlerton *hton, TABLE_SHARE *table_arg);

	~ha_protXML_mod_aminoacid_mass();

	const char *table_type() const { return "PROTXML_MOD_AMINOACID_MASS"; }
...
enum protXML_mod_aminoacid_mass_field { GROUP_NUMBER = 0, PSEUDO_NAME = 1, PROBABILITY = 2, DONE = 3 };

struct protXML_mod_aminoacid_mass_field_info {
	const char *fieldname;
	enum protXML_mod_aminoacid_mass_field state;
	enum protXML_mod_aminoacid_mass_field next_state;
	uint type;
	const char *type_text;
} protXML_mod_aminoacid_mass_fields[] = { \
