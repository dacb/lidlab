...
#define PROTXML_PROTEIN_GROUP_DEFAULT_ROOT_ALLOC 8192
...
typedef struct st_protXML_protein_group_share {
	char *table_name;
	...
	struct st_protXML_protein_group_share_position {
		int end;
	} filepos;
	...
	THR_LOCK lock;
} PROTXML_PROTEIN_GROUP_SHARE;

class ha_protXML_protein_group: public handler {
	THR_LOCK_DATA lock;	/* MySQL based lock for handler */
	PROTXML_PROTEIN_GROUP_SHARE *share;	/* shared lock */

	int init_data_file();
	File protXML_protein_group_file;
...
public:
	ha_protXML_protein_group(handlerton *hton, TABLE_SHARE *table_arg);

	~ha_protXML_protein_group();

	const char *table_type() const { return "PROTXML_PROTEIN_GROUP"; }
...
enum protXML_protein_group_field { GROUP_NUMBER = 0, PSEUDO_NAME = 1, PROBABILITY = 2, DONE = 3 };

struct protXML_protein_group_field_info {
	const char *fieldname;
	enum protXML_protein_group_field state;
	enum protXML_protein_group_field next_state;
	uint type;
	const char *type_text;
} protXML_protein_group_fields[] = { \
