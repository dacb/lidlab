...
#define PROTXML_PEPTIDE_PARAMETER_DEFAULT_ROOT_ALLOC 8192
...
typedef struct st_protXML_peptide_parameter_share {
	char *table_name;
	...
	struct st_protXML_peptide_parameter_share_position {
		int end;
	} filepos;
	...
	THR_LOCK lock;
} PROTXML_PEPTIDE_PARAMETER_SHARE;

class ha_protXML_peptide_parameter: public handler {
	THR_LOCK_DATA lock;	/* MySQL based lock for handler */
	PROTXML_PEPTIDE_PARAMETER_SHARE *share;	/* shared lock */

	int init_data_file();
	File protXML_peptide_parameter_file;
...
public:
	ha_protXML_peptide_parameter(handlerton *hton, TABLE_SHARE *table_arg);

	~ha_protXML_peptide_parameter();

	const char *table_type() const { return "PROTXML_PEPTIDE_PARAMETER"; }
...
enum protXML_peptide_parameter_field { GROUP_NUMBER = 0, PSEUDO_NAME = 1, PROBABILITY = 2, DONE = 3 };

struct protXML_peptide_parameter_field_info {
	const char *fieldname;
	enum protXML_peptide_parameter_field state;
	enum protXML_peptide_parameter_field next_state;
	uint type;
	const char *type_text;
} protXML_peptide_parameter_fields[] = { \
