...
#define TEMPLATE_DEFAULT_ROOT_ALLOC 8192
...
typedef struct st_template_share {
	char *table_name;
	...
	struct st_template_share_position {
		my_off_t end;
	} filepos;
...
class ha_template: public handler {
	THR_LOCK_DATA lock;	/* MySQL based lock for handler */
	TEMPLATE_SHARE *share;	/* shared lock */
	...
	struct ha_template_file_position {
		my_off_t current;
		my_off_t next;
		my_off_t end;
	} filepos;
...
public:
	ha_template(handlerton *hton, TABLE_SHARE *table_arg);
	...
	const char *table_type() const { return "TEMPLATE"; }
