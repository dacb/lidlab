...
#define BLAST_FMT6	".blast_fmt6"

#define BLAST_FMT6_DEFAULT_ROOT_ALLOC 8192
...
typedef struct st_blast_fmt6_share {
	char *table_name;
	...
	struct st_blast_fmt6_share_position {
		my_off_t end;
	} filepos;
...
class ha_blast_fmt6: public handler {
	THR_LOCK_DATA lock;	/* MySQL based lock for handler */
	BLAST_FMT6_SHARE *share;	/* shared lock */

	int init_data_file();
	Transparent_file *file_buf;
	File blast_fmt6_file;

	/* position data for reordering */
	struct ha_blast_fmt6_file_position {
		my_off_t current;
		my_off_t next;
		my_off_t end;
	} filepos;
...
public:
	ha_blast_fmt6(handlerton *hton, TABLE_SHARE *table_arg);
	...
	const char *table_type() const { return "BLAST_FMT6"; }
